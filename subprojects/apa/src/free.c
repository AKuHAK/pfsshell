...
static void apaCalculateFreeSpace(u32 *free, u32 sectors)
{
    ...
    for (sectors /= 2; 0x3FFF < sectors; sectors /= 2)
        *free |= sectors;
}
...
int apaGetFreeSectors(s32 device, u32 *free, apa_device_t *deviceinfo)
{
    ...
    *free = 0;
    ...
    if (rv == 0) {
        for (partMax = deviceinfo[device].partitionMaxSize; 0x0003FFF < partMax; partMax = deviceinfo[device].partitionMaxSize) { //As weird as it looks, this was how it was done in the original HDD.IRX.
            for (; 0x0003FFF < partMax; partMax /= 2) {
